...
def _dnsdumpster_subdomains(
    ...
    Raises DnsDumpsterQuotaError when the daily quota is exhausted.
    """
    if not HAS_REQUESTS:
        return []
...
def _dnsdumpster_api(
    ...
    while True:
        ...
        # ── Quota detection: inspect raw text before any status branching ──────
        # DNSDumpster sends {"error":"Daily quota exceeded"} as the body of a
        # 429. Check the raw text first — before JSON parsing and before the
        # 429 retry branch — so we never loop on a quota response.
        ...
        if resp.status_code == 429:
            # Genuine per-request rate limit (body had no quota language)
            logger.warning(
...
def _passive_dns_enum(domain: str, ns_hosts: list,
                      timeout: float = 5.0) -> list:
    """
    Passive DNS fallback: SRV records, AXFR attempt, PTR reverse lookup.
    No external APIs — dnspython only. Always non-intrusive.
    Returns deduplicated, sorted list of FQDNs that are children of *domain*.
    """
    if not HAS_DNSPYTHON:
        return []
    ...
    for ns in ns_hosts:
        ns_clean = ns.rstrip(".")
        try:
            ns_ips = _resolve(ns_clean, "A", timeout) or _resolve(ns_clean, "AAAA", timeout)
            if not ns_ips:
                continue
            logger.debug(f"Attempting AXFR for {domain} from {ns_clean}")
            xfr = dns.query.xfr(ns_ips[0], domain, timeout=timeout, lifetime=timeout * 2)
            z = dns.zone.from_xfr(xfr)
            for name in z.nodes:
                fqdn = str(name)
                if fqdn != "@":
                    fqdns.add(f"{fqdn}.{domain}".lower())
                    logger.info(f"AXFR {domain} via {ns_clean}: found {fqdn}.{domain}")
        except Exception as exc:
            logger.debug(f"AXFR refused/failed for {domain} from {ns_clean}: {exc}")

    # ── PTR reverse lookup of apex A records ──────────────────────
    import socket as _socket
    for ip in _resolve(domain, "A", timeout)[:4]:
        try:
            hostname = _socket.gethostbyaddr(ip)[0].lower().rstrip(".")
...
def enumerate_domain(
    ...
    use_passive = not use_dnsdumpster  # always run passive when DD is off
    if use_dnsdumpster:
        try:
            ...
            logger.info(
                ...
                "running passive DNS fallback (SRV + AXFR attempt + PTR)"
            )
            use_passive = True
        except Exception as exc:
    ...
    # ── Passive DNS (SRV / AXFR / PTR) ───────────────────────────
    if use_passive:
        try:
            passive_subs = _passive_dns_enum(domain, result.ns_hosts, timeout)
            if passive_subs:
                logger.info(f"{domain}: passive DNS found {len(passive_subs)} additional hosts")
